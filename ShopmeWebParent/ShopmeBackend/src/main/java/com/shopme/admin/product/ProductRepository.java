...
public interface ProductRepository extends PagingAndSortingRepository<Product, Integer> {
  ...
  @Query(
      "SELECT p FROM Product p WHERE (p.category.id = ?1 OR p.category.allParentIDs LIKE %?2%) AND "
  ...
  Page<Product> searchInCategory(
      Integer categoryId, String categoryIdMatch, String keyword, Pageable pageable);
}
